...
export async function POST(request: NextRequest) {
    try {
        ...
        const answeredQuestions = (session.questions || []).filter((q: any) => q.answer_transcript);

        if (answeredQuestions.length === 0) {
            return NextResponse.json({ error: "No answers to generate report from" }, { status: 400 });
        }

        const questionsText = answeredQuestions.map((q: any, i: number) => `
Q${i + 1}: ${q.question}
...
Return this exact JSON:
{
  ...
  "recommendation": "<Strong Hire|Hire|Maybe|No Hire>"
        ...
        return NextResponse.json({ reportId: report.id, report: { ...reportData, breakdown } });
    } catch (error: any) {
