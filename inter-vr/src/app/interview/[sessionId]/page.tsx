...
import {
    Mic, MicOff, Square, Volume2, ChevronRight, Loader2,
    CheckCircle2, AlertCircle, Brain, Clock, BarChart2, Trophy
} from "lucide-react";
...
export default function InterviewRoomPage() {
    ...
    // ── Timer ──────────────────────────────────────────────────────────────────
    useEffect(() => {
        if (phase === "listening" || phase === "recording" || phase === "speaking") {
    ...
    useEffect(() => {
        const load = async () => {
            ...
            const { data: ctx } = await supabase
                .from("users")
                .select("full_name, institution_name, year_of_study, tech_stack, processed_resume")
                .eq("id", user.id)
                .single();
    ...
    const speak = useCallback(async (text: string): Promise<void> => {
        return new Promise((resolve) => {
            ...
            utterance.onend = () => { setAudioPlaying(false); resolve(); };
    ...
    const submitAnswer = async () => {
        ...
        if (!transcribedText && audioBlob.size > 0) {
            try {
                ...
                const sttRes = await fetch("/api/interview/stt", { method: "POST", body: formData });
    ...
    // ── UI ─────────────────────────────────────────────────────────────────────
    const progressPct = questions.length > 0 ? ((currentQIndex) / questions.length) * 100 : 0;
    const avgScore = answeredCount > 0 ? Math.round((cumulativeScore / answeredCount) * 10) : 0;
    ...
    if (phase === "error") return (
        <div className="min-h-screen bg-slate-950 flex items-center justify-center">
            <div className="flex flex-col items-center gap-4 text-center max-w-md">
                ...
                <button onClick={() => router.push("/dashboard")} className="mt-4 px-6 py-3 bg-brand-purple rounded-xl text-white font-semibold">Back to Dashboard</button>
            </div>
        </div>
    );
    ...
    if (phase === "completed") return (
        <div className="min-h-screen bg-slate-950 flex items-center justify-center">
            <div className="flex flex-col items-center gap-6 text-center max-w-md px-4">
                ...
                <p className="text-slate-400">You answered {answeredCount} questions with an average score of <span className="text-brand-neon font-bold">{avgScore}%</span></p>
                {reportId ? (
                    <button
                ...
                <button onClick={() => router.push("/dashboard")} className="text-slate-500 hover:text-slate-300 text-sm transition">Back to Dashboard</button>
            </div>
        </div>
    );
        ...
        <div className="min-h-screen bg-slate-950 text-slate-50 flex flex-col">
            ...
            <div className="border-b border-slate-800/60 bg-slate-900/50 backdrop-blur px-6 py-4 flex items-center justify-between">
                <div className="flex items-center gap-3">
                    ...
                    <span className="font-bold text-white">InterVR <span className="text-slate-500 font-normal text-sm">/ Live Session</span></span>
                </div>
                <div className="flex items-center gap-6 text-sm text-slate-400">
                    ...
                    <div className="flex items-center gap-1.5">
                        ...
                        <span>Q {Math.min(currentQIndex + 1, questions.length)}/{questions.length}</span>
                    </div>
                    {answeredCount > 0 && (
            ...
            <div className="flex-1 flex flex-col items-center justify-center px-4 py-12 max-w-3xl mx-auto w-full gap-8">
                ...
                {phase === "ready" && (
                    <div className="text-center space-y-6">
                        ...
                        <p className="text-slate-400">{session?.topic} • {session?.difficulty} • {session?.duration}</p>
                        <p className="text-slate-400 text-sm">{questions.length} questions prepared. The AI interviewer will speak each question aloud.</p>
                        <button
                ...
                {(phase === "speaking" || phase === "listening" || phase === "recording" || phase === "processing" || phase === "feedback" || phase === "followup") && currentQuestion && (
                    <>
                        {/* Question Display */}
                        <div className="w-full bg-slate-900/60 border border-slate-800 rounded-2xl p-6 space-y-3 backdrop-blur">
                            <div className="flex items-center gap-2">
                                ...
                                <span className="text-xs px-2 py-0.5 bg-slate-800 rounded-full text-slate-400">{currentQuestion.category}</span>
                                <span className="text-xs px-2 py-0.5 bg-slate-800 rounded-full text-slate-400">{currentQuestion.difficulty}</span>
                            </div>
                        ...
                        {phase === "speaking" && (
                            <div className="flex flex-col items-center gap-4">
                                ...
                                <div className="flex items-center gap-1">
                                    {[1, 2, 3, 4, 5].map(i => (
                                        <div key={i} className="w-1 bg-brand-purple rounded-full animate-bounce"
                                ...
                                <div className="flex items-center gap-1">
                                    {[1, 2, 3, 4, 5, 6, 7].map(i => (
                                        <div key={i} className="w-1 bg-red-500 rounded-full animate-bounce"
                                            style={{ height: `${6 + Math.sin(i) * 10 + 10}px`, animationDelay: `${i * 0.08}s` }} />
                                    ))}
                                </div>
                                <button
